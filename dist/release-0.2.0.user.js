...
var rollupUserScript = (function (React, require$$0) {
  ...
  */
  ...
  // REGEXES
  const TEXT_INSIDE_PARANTHESIS_REGEX = /\(([^)]+)\)(?![^(]*\))/g;
   ...
   */
  async function getWebLinksPayload(weblinks, linkedGroupWeblinksFormData, platform) {
  ...
  async function copyWeblinksToLinkedGroup(weblinks, linkedGroup) {
      ...
      const payload = await getWebLinksPayload(weblinks, linkedGroupWeblinksFormData, linkedGroup.title);
  ...
  async function getGameFaqLinkByPlatform(url, platform) {
      ...
      const parser = new DOMParser();
      const parsedHTML = parser.parseFromString(rawHTML, "text/html");
      const anchorElements = parsedHTML.querySelectorAll(GAMEFAQ_PLATFORM_LINK_SELECTOR);
              ...
              for (const { id } of collections) {
                  ...
                  await addTorrentsToCollection(id, filteredDestinationLinkedGroups
                      ...
                      if (PC_LINKED_GROUPS_TITLES.includes(title) &&
                          EXCLUDED_PC_COLLECTION_IDS.includes(id)) {
                          return false;
                      }
