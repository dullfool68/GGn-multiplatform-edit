...
var rollupUserScript = (function (React, require$$0) {
  ...
  */
  ...
  // WEBLINKS
  const EXCLUDED_WEBLINKS = ["Amazon", "GameFAQs"];
  // MAPPING
  /**
      ...
      async function handleRun() {
          ...
          try {
              ...
              for (const { id } of collections) {
                  await addTorrentsToCollection(id, destinationLinkedGroups
                      .filter((_, index) => areAllCheckboxesUnchecked ? true : checkboxes[index])
                      .filter(({ title }) => {
